...
class POIPage extends LitElement {
  ...
  connectedCallback() {
    ...
    // Extract poiid from URL path: /poi/{poiid}.html
    const parts = location.pathname.split("/").filter(Boolean);
    const poiidWithExt = parts[1];
    this.poiid = poiidWithExt?.replace(/\.html$/, "") || "";
  ...
  renderBreadcrumb() {
    ...
    const parkLink = this.poi?.park
      ? `/parks/${this.poi.park}/index.html`
      : "#";
