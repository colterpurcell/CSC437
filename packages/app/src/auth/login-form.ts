...
export class LoginFormElement extends LitElement {
  ...
  handleChange(event: Event) {
    ...
    // sync disabled state of any slotted action buttons
  ...
  handleSlotChange() {
    ...
    // Wire up any provided slotted button to submit the form
    const buttons = this.querySelectorAll('button[slot="button"]');
    buttons.forEach((btn) => {
      // Avoid duplicate listeners by resetting first
      btn.replaceWith(btn.cloneNode(true));
    });
    const freshButtons = this.querySelectorAll('button[slot="button"]');
  ...
  private syncButtonDisabled() {
    const buttons = this.querySelectorAll('button[slot="button"]');
    buttons.forEach((btn) => {
      (btn as HTMLButtonElement).disabled = !this.canSubmit;
    });
  }
}
