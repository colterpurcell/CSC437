...
class CampsitePage extends LitElement {
  ...
  connectedCallback() {
    ...
    if (!this.siteIdAttr) {
      console.error("<campsite-page> requires site-id attribute");
      this.loading = false;
      this.error = "Missing site-id";
      return;
    }
    this.siteid = this.siteIdAttr;
    ...
    return html`
      ...
      <div class="chip-row">
        ${paths.length > 0
          ? paths.map(
              (path) => html`
                <a class="chip" href="/app/paths/${path.pathId}">
                  <svg class="icon icon-sm">
                    <use
    ...
    return html`
      ...
      <div class="chip-row">
        ${poi.length > 0
          ? poi.map(
              (poi) => html`
                <a class="chip" href="/app/poi/${poi.poiId}">
                  <svg class="icon icon-sm">
                    <use href="/assets/icons/camping.svg#icon-pin"></use>
                  </svg>
                  ${poi.poiName}
