...
class ParkPage extends LitElement {
  ...
  connectedCallback() {
    ...
    if (!this.parkIdAttr) {
      console.error("<park-page> requires park-id attribute");
      this.loading = false;
      this.error = "Missing park-id";
      return;
    }
    this.parkid = this.parkIdAttr;
        ...
        ${this.campsites.map(
          (campsite) => html`
            <card-element
              ...
              ""}"
              href="/app/campsites/${campsite.siteid}"
              clickable
